...
AVAILABLE_CATEGORIES = [
    ...
    'DepartmentStores', 'CafesResturantsAndTakeawayFood', 'Other'
]
...
AVAILABLE_COMMODITIES = [
    'Total','FoodAndLiveAnimals', 'BeveragesAndTobacco', 'CrudMaterialAndInedible',
    'MineralFuelLubricentAndRelatedMaterial', 'AnimalAndVegitableOilFatAndWaxes', 'ChemicalsAndRelatedProducts',
    'ManufacutedGoods', 'MachineryAndTransportEquipments', 'OtherManucacturedArticles', 'Unclassified'
]
...
class UserBehavior(TaskSet):
    ...
    def invalid_state(self):
        url = '/api/api.php?StatisticsArea=Retail&Category=Food&State=USA&startDate=2016-01-01&endDate=2016-03-01'
    ...
    def invalid_category(self):
        url = '/api/api.php?StatisticsArea=Retail&Category=Foodie&State=NSW&startDate=2016-01-01&endDate=2016-03-01'
        with self.client.get(url, catch_response=True) as response:
            if response.status_code == 500:
                response.success()
            else:
    ...
    def random_query_merch(self):
        ...
        url = '/api/api.php?StatisticsArea=MerchandiseExports&Category={com}&State={states}&startDate=2016-03-01&endDate=2017-03-01'\
    ...
    def random_query_retail(self):
        ...
        states = random.sample(AVAILABLE_STATES, random.randint(1, NUM_STATES))
        url = '/api/api.php?StatisticsArea=Retail&Category={cat}&State={states}&startDate=2016-03-01&endDate=2017-03-01'.format(
    ...
    def invalid_date_range(self):
        ...
        :return: 
        """
        url = '/api/api.php?StatisticsArea=Retail&Category=Food&State=NSW&startDate=2017-01-01&endDate=2016-01-01'
        with self.client.get(url, catch_response=True) as response:
            if response.status_code == 500:
                response.success()
            else:
    ...
    def invalid_dates(self):
        ...
        :return: 
        """
        url = '/api/api.php?StatisticsArea=Retail&Category=Food&State=NSW&startDate=2016-02-31&endDate=2016-01-01'
        with self.client.get(url, catch_response=True) as response:
            if response.status_code == 500:
                response.success()
            else:
    ...
    def check_simple_output(self):
        ...
        :return: 
        """
        url = '/api/api.php?StatisticsArea=Retail&Category=Food&State=AUS&startDate=2016-01-01&endDate=2016-03-01'
        with self.client.get(url, catch_response=True) as response:
            ...
            try:
                ...
                elif not result['MonthlyRetailData'][0]['RegionalData'][0]['Data'][0]['Date'] == 'Jan-2016':
                    response.failure('Wrong starting date of returns')
                elif not result['MonthlyRetailData'][0]['RegionalData'][0]['Data'][2]['Date'] == 'Mar-2016':
