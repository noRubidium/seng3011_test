...
*** Variables ***
${API_ENDPOINT}               http://45.76.114.158/api?
${VERSION}                    v4
...
M.1.1 Merch All Variables No Dates
  ...
  Create API Context
  GET  							StatisticdArea=${stats_area}&State=${states}&Category=${categories}&startDate=${start_date}&endDate=${end_date}
  Log								${API_ENDPOINT}StatisticdArea=${stats_area}&State=${states}&Category=${categories}&startDate=${start_date}&endDate=${end_date}
  ...
  ${json_dict} =      evaluate        json.loads('''${exp_json}''')   json
  set to dictionary   ${json_dict}    header=null
  ${exp_json} =       evaluate        json.dumps(${json_dict})
